...
static int request(uint16_t port, const char *method, const char *target,
    ...
    if (strcmp(target, "/cookie-options") == 0)
        framework_headers_ok = strstr(
            response,
            "Set-Cookie: theme=aster; Path=/account; Domain=lime.test; Max-Age=3600; Secure; SameSite=Strict\r\n"
        ) != NULL;
...
int main(int argc, char **argv) {
    ...
    if (child == 0) {
        ...
        if (strcmp(argv[1], "vm") == 0) {
            ...
            _exit(lang_project_run_ir(
                "packages/lime/aster.toml", "current_http_server"));
        }
    ...
    if (result == 0)
        result = request(
            ...
            "title=Lime+forms%21&kind=client+site", 200,
            "text/html; charset=utf-8",
            "<p>Lime forms!:client site</p>");
    if (result == 0)
        result = request(
    ...
    if (result == 0)
        result = request(
            port, "POST", "/submit",
            "multipart/form-data; boundary=lime-test",

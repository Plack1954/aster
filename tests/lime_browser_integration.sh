...
grep -Eq 'data-aster-project="t:[0-9a-z]{1,13}"' <<<"$page"
grep -Eq 'data-aster-project="d:[0-9a-z]{1,13}"' <<<"$page"
grep -Eq 'data-aster-project="c:[0-9a-z]{1,13}"' <<<"$page"
grep -Eq 'data-aster-event="click\|SelectSecondAndRemoveFirst\|p\|l:@[0-9a-z]{1,13}"' <<<"$page"
...
grep -q 'click|PersistentTodoList_AppendTodo|v|x:PersistentTodoList' <<<"$page"
grep -q 'click|PersistentTodoList_RenameTodo|v|x:PersistentTodoList|s:key' <<<"$page"
grep -Eq 'data-aster-part-t="[0-9a-z]{1,13}"' <<<"$page"
grep -Eq 'data-aster-part-c="[0-9a-z]{1,13}"' <<<"$page"
grep -Eq 'data-aster-part-d="[0-9a-z]{1,13}"' <<<"$page"
grep -Eq 'data-aster-part-h="[0-9a-z]{1,13}"' <<<"$page"
grep -Eq 'data-aster-part-a="[0-9a-z]{1,13}' <<<"$page"
grep -q 'data-aster-component-list-state=' <<<"$page"
grep -Eq 'data-aster-state-field-[0-9a-z]{1,13}' <<<"$page"

...
static int session_roundtrip(uint16_t port) {
    ...
    if (result != 0 || strstr(first, "HTTP/1.1 200 ") == NULL ||
        ...
        return 1;
    static const char prefix[] = "set-cookie: aster.session=";
    ...
    if (result != 0 || strstr(second, "HTTP/1.1 200 ") == NULL ||
        ...
        find_ascii_case(second, "set-cookie: aster.session=") != NULL)
        return 5;
    return 0;
}
...
int main(int argc, char **argv) {
    ...
    if (result == 0)
        result = request(
            ...
            "title=Fast+Aster+Web", NULL, 200, "text/html; charset=utf-8",
            "<p>Fast Aster Web</p>", NULL);
    ...
    if (result == 0)
        result = request(
            ...
            "Aster Web:mark.svg:image/svg+xml:22", NULL);
    if (result == 0)
        result = request(

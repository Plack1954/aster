# Deploying Aster Web behind nginx

The normal Aster Web production boundary is:
...
instead of preserving an untrusted client-supplied chain. Aster Web accepts those
...
so Aster Web streaming responses remain streaming.
...
`restart` sends SIGTERM to the old process. Aster Web closes its listener, asks H2O

# Aster Web Minimal API mission

## Decision

Aster Web is Aster's explicit, transport-neutral HTTP framework. Its primary public
reference is ASP.NET Core Minimal APIs. Aster Web will not implement controllers,
...
runtime. Aster Web should copy the parts that make small HTTP applications direct:
...
The source confirms four architectural choices for Aster.Web. A route pattern is a
...
Aster Web will not acquire any of the following as a consequence of this mission:
...
Aster Web's transport boundary is substantially better than its application API.
...
- Explicit route matching, static files, filters, and fallback behavior remain
  inside Aster Web and therefore behave consistently across adapters.
- SSG executes the real GET route graph instead of introducing a separate page
  framework.
- Application state is explicit; Aster Web has no global service container.
...
- `Aster.Web.Routing.RoutePattern` parses route templates during registration.
...
- Aster Web's registration surface is now `MapGet`, `MapPost`, `MapPut`, `MapPatch`,
...
- `Aster.Web.Ssg.SiteBuildAsync` dispatches the same async endpoint graph during
...
become permanent framework architecture. Aster Web can migrate stateful endpoints
...
The initial grammar is an ordinary string grammar owned by Aster Web:
...
always direct, typed request access; Aster Web must not accumulate invented wrapper
...
Aster Web must not silently insert authentication, authorization, CORS, or forwarded
header behavior based on package presence.

## Results and responses

Aster Web should retain its transport-neutral `Response` value and typed body union.
...
Aster Web should learn from `TypedResults`: concrete result types can carry useful
...
- path matching uses Aster Web's adapter-neutral normalized path, never the query
...
- Support explicit query, header, body, form, and request abstractions only
  after each API is proven against real Aster Web applications.
...
- Extend the implemented borrowed `EndpointDataSource` only when real tooling
  proves that additional metadata is necessary. The bounded `Aster.Web.OpenApi`
...
- Convert Aster Web tests, the blog fixture, Nook, docs-server, and issue-tracker

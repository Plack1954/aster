# Aster Web browser client and Wasm direction

## Direction

Aster Web Browser should add typed interaction to ordinary server-rendered or
...
in [Composable, compiler-checked browser projections](aster-web-composable-projections.md).
...
Compiler part identity is a stable 64-bit hash encoded as at most 13 base-36
characters in HTML, state schemas, event bindings, and compatibility Wasm
batches; application field names are not used for matching.

Dynamic text mixed with nested elements uses compiler-emitted comment ranges,
so updates replace only the scalar text between markers and retain nested nodes.
Safe metadata attributes and CSS custom properties are inferred parts as well.
Form values, checked state, selection, focus, and caret state remain browser-owned
unless a future explicit controlled-property spelling opts in.

Structural and conditional whole-root changes, nested state structs, multiple
list fields, and URL-bearing or browser-mutable inferred attributes remain
...
[`aster-web-component-semantics.md`](aster-web-component-semantics.md).

## A likely layered model

Aster Web may not need one universal browser update mechanism. A coherent hierarchy
could be:
...
Browser interaction is an enhancement of the normal Aster Web application:

- SSR and SSG produce the initial native HTML;
- ordinary links, forms, actions, and methods remain meaningful;
- the complete Aster Web server application remains available;
...
[`aster-web-final-measurement.md`](aster-web-final-measurement.md).
...
[`aster-web-final-application-proof.md`](aster-web-final-application-proof.md). It covers
...
9. Development speed and application ceremony are measured alongside runtime
   performance.
10. Browser work strengthens the normal Aster Web server application rather than

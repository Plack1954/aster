# Lime seamless SSR and static generation

## Direction

Lime is not an SSG framework and should not become an Aster version of Astro.
...
> Every Lime application remains a complete runnable SSR application.
...
Lime should not adopt the parts of Astro that conflict with Aster's model:
...
The current `Lime.Ssg` implementation already follows the correct semantic
shape:
...
entries is unavoidably required. Lime's existing `BuildSource` is a valid
minimal answer:
...
frontmatter, JSON, SQLite, HTTP, generated values, or hardcoded records. Lime
...
The third mode is an output choice, not the identity of the application. Lime
should optimize primarily for the first two because Aster/Lime applications
...
Lime should infer only behavior that is obvious and explainable:
...
Explicit controls should be uncommon escape hatches. Lime should avoid opaque
...
metadata should generally be normal reusable Aster/Lime libraries rather than
...
Lime should retain explicit code routing and typed endpoint metadata rather

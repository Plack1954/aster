# Aster Web HTTP adapters

## Roles

Aster Web application code is independent of the HTTP implementation. Routes,
forms, sessions, middleware, exception handling, static-file policy, and typed
responses belong to Aster.Web. Connection management and protocol mechanics belong
to an adapter.

`Aster.Web.CurrentHttp` is the development and conformance adapter. It is a small,
...
unless they are required to test Aster Web's adapter contract; correctness and
security defects are still fixed.

`Aster.Web.H2O` is the production-adapter track. It embeds libh2o and must preserve
the same Aster Web `Request`, `Response`, routing, and exception semantics. H2O owns
HTTP parsing, connection persistence, protocol negotiation, transport
timeouts, and response I/O. Aster Web owns application decisions.
...
4. Send Aster Web's validated status, headers, cookies, and typed content kind.
...
Aster Web roadmap. See `docs/aster-web-nginx-deployment.md`.
...
With `HandleSignals` enabled, SIGINT and SIGTERM stop the accept loop. Aster Web
...
H2O can make several parsed requests ready during one event-loop turn. Aster Web
...
`tools/build_aster_web_h2o_app.sh` emits and compiles an application target into a
...
in `docs/aster-web-nginx-deployment.md`.

Aster Web `Response.Stream` uses an H2O generator. Aster reads at most one 64 KiB
...
`stop` callback turns a disconnect into an ordinary adapter error, and Aster Web's
...
A mounted static file becomes an adapter-neutral Aster Web `Response.File`; the
application does not name an H2O API. Filters, explicit routes, static-path
validation, fallback behavior, and SSG discovery still execute in Aster.Web.

Before its first accept, `H2OBind` or `H2OBindStateful` registers the roots
already present in that Aster Web route graph. When Aster Web selects a file response,
...
second public routing surface or bypass Aster Web's request filters.
...
Aster Web response header when delegating the file to its native file handler.

# Lime application baseline

## Status

The source under `packages/lime/src/examples/blog` is a routing and SSG test
fixture. It must not be described as a complete blog, a showcase application,
or evidence that Lime is pleasant to use. It contains one hardcoded post and
...
This target was created independently of Lime. It therefore cannot be shaped
to flatter Lime or demonstrate only features Lime already has.
...
targets for Aster. Lime succeeds by expressing the same application with less
...
SSR-first Lime application using the supplied Markdown, CSS, JavaScript, fonts,
...
Lime now renders the configured SSR fallback through the normal filters and
HTML middleware and writes it directly to root `404.html`. Nook no longer
registers a fake `/404/` page for static output.

Aster manifests now resolve Lime as a local path dependency, and Nook no
longer contains a source symlink. A single Lime static-directory declaration
...
Lime now owns the reusable frontmatter document parser and safe bounded
...
The port also found and fixed two concrete platform defects: Lime served CSS

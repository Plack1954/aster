...
with an operation stream at the Wasm boundary. It is not Incremental DOM: Aster Web
should not rerun an entire render function and walk every element after each
event. It is not Solid's model: Aster Web does not currently need runtime signals,
...
Difference: Lit reevaluates JavaScript template expressions. Aster Web must evaluate
...
Difference: modern Svelte also has a substantial reactive runtime model. Aster Web
...
Useful lesson for Aster Web: signals solve dependency discovery, not DOM binding or
...
Useful lesson for Aster Web: a linear operation stream is a good boundary between
Wasm and JavaScript.

Why not use its rendering algorithm directly:

- rerunning a complete render still visits unrelated elements;
- Aster Web already knows many exact projection targets;
...
comparison shows that Aster Web can be much smaller and faster for some direct
operations, but Vue currently wins breadth and some bulk operations.

Aster Web should copy the capability requirements, not Vue's VDOM architecture.
...
properties change, direct parts update. If the active branch changes, Aster Web
...
small mutation observer may manage region lifetime, or Aster Web may constrain
...
Only after these prototypes should Aster Web select source syntax and make a public
API commitment.

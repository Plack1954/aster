...
Aster Web is Aster's explicit web framework. It provides transport-neutral routing,
...
Aster Web applications use the same endpoint graph with the in-memory dispatcher,
...
Aster Web supplies `MapGet`, `MapPost`, `MapPut`, `MapPatch`, `MapDelete`,
...
The optional `Aster.Web.OpenApi` module generates a bounded OpenAPI 3.1 document
...
query/header, response-content, or component schemas that Aster Web's endpoint
...
Aster Web also has a bounded, explicit multi-source binding surface. Source markers
are passed at registration; Aster Web does not guess a source from the handler's
parameter type:
...
suppresses the body. Aster Web answers automatic OPTIONS requests with 204 and
...
`BodyBytes` is a zero-copy borrowed view over Aster Web's owned buffered request
...
limits still bound buffering before Aster Web dispatch (for H2O, configure
...
- `Aster.Web.Forms` parses URL-encoded and multipart forms. Multipart parsing walks
  ...
  spill file.
- `Aster.Web.Sessions` provides explicit server-side sessions. Session identifiers
...
- `Aster.Web.Static` mounts safe static-file roots.
- `Aster.Web.Browser` adds optional browser/Wasm assets and hydration support. Its
  retained-DOM direction is documented in
  [`../../docs/aster-web-browser-client.md`](../../docs/aster-web-browser-client.md).
- `Aster.Web.Ssg` executes the real GET endpoint graph to produce static output. Its
  seamless SSR/publication direction is documented in
  [`../../docs/aster-web-seamless-ssg.md`](../../docs/aster-web-seamless-ssg.md).
- `Aster.Web.Content` and `Aster.Web.Markdown` support content-oriented sites.
- `Aster.Web.CurrentHttp` is the development and conformance adapter.
- `Aster.Web.H2O` is the production adapter track.

Transport responsibilities and production deployment are documented in
[`../../docs/aster-web-http-adapters.md`](../../docs/aster-web-http-adapters.md) and
[`../../docs/aster-web-nginx-deployment.md`](../../docs/aster-web-nginx-deployment.md).
The Minimal API restructuring mission is specified in
[`../../docs/aster-web-minimal-api-mission.md`](../../docs/aster-web-minimal-api-mission.md).

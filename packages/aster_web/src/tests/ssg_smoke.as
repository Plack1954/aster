...
private Response home(const ref Request request)
{
    return Results.Html(
        <html>
            <head><title>Aster Web SSG</title></head>
...
private Result<int, string> build()
{
    ...
    List<ContentDocument> documents = LoadContentDirectory(
        "packages/aster_web/test_content", ".md"
    );
    ...
    if (discovered.ToString() !=
        "packages/aster_web/test_content/a.md\npackages/aster_web/test_content/b.md\n")
    {
        return Result.Err(
    ...
    app.Static("/assets/", "packages/aster_web/test_assets");

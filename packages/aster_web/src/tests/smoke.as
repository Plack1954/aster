...
using Aster.Web.Forms;
using Aster.Web.Forwarding;
using Aster.Web.Content;
using Aster.Web.Markdown;
using Aster.Web.Static;
using Aster.Memory;
using Aster.Html;
using System.IO;
using System.Text;
...
{
    switch (TryLoadContentDirectory("packages/aster_web/test_content", ".md"))
    {
...
private bool ProblemResultsWork()
{
    ...
    problem.Type = "https://aster.test/problems/conflict";
    ...
    switch (body)
    {
        case ResponseBody.Asset(asset): {
            ...
            return kind == AssetKind.ProblemJson && bytes ==
                "{\"type\":\"https://aster.test/problems/conflict\",\"title\":\"Conflict\",\"status\":409,\"detail\":\"The article already exists.\",\"instance\":\"/articles/aster\"}";
        }
        case ResponseBody.Empty: { return false; }
        case ResponseBody.Html(page): { return false; }
        case ResponseBody.Text(text): { return false; }
...
{
    switch (StaticFile("packages/aster_web/testdata", "/../README.md"))
    {
        case Result.Ok(response): { return false; }
        case Result.Err(error): {
        }
    }
    switch (StaticFile("packages/aster_web/testdata", "/mark.svg"))
    {
        case Result.Err(error): { return false; }
...
{
    switch (StaticFile("packages/aster_web/testdata", "/site.css"))
    {
        case Result.Err(error): { return false; }
        case Result.Ok(response): {
            (int status, ResponseBody body,
...
private bool StaticDirectoriesWork()
{
    ...
    WebApplication app = appOwner.Value;
    app.MapFallback(missing);
    switch (app.TryStatic("/assets/", "packages/aster_web/testdata"))
    {
        case Result.Err(error): { return false; }
...
private bool StaticCachePolicyWorks()
{
    ...
    WebApplication app = appOwner.Value;
    app.MapFallback(missing);
    app.Static("/assets/", "packages/aster_web/testdata", options);
    Response response = app.Dispatch(request("GET", "/assets/site.css"));
    (int status, ResponseBody body, List<ResponseHeader> headers) = response;
    ...
    switch (app.TryStatic("/bad/", "packages/aster_web/testdata", invalid))
    {
...
private bool MultipartFormsWork()
{
    ...
    uploadBody.Append("--aster-web-boundary\r\n");
    uploadBody.Append(
        "Content-Disposition: form-data; name=\"title\"\r\n\r\n"
    );
    uploadBody.Append("Aster & Aster Web\r\n");
    uploadBody.Append("--aster-web-boundary\r\n");
    uploadBody.Append(
    ...
    Request upload = RequestNew(
        ...
        "/upload",
        "example.test",
        "multipart/form-data; boundary=aster-web-boundary",
        "",
        body
    );
    ...
    switch (form.Get("title"))
    {
        case Option.Some(title): {
            if (title != "Aster & Aster Web") { return false; }
        }
        case Option.None: { return false; }
    }
...
private bool BoundServiceRoutes()
{
    ...
    {
        Value = new SiteState("Stateful Aster Web")
    };

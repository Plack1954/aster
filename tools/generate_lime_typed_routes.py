#!/usr/bin/env python3
"""Generate Lime's explicit typed-route overload matrices."""
...
LIME = ROOT / "packages" / "lime" / "src" / "lime.as"
...
def main():
    ...
    parser.add_argument(
        "--check",
        action="store_true",
        help="fail if lime.as is not synchronized with this registry",
    )
    arguments = parser.parse_args()

    original = LIME.read_text(encoding="utf-8")
    ...
    if arguments.check:
        if generated != original:
            raise SystemExit("Lime typed-route overloads are out of date")
        return
    LIME.write_text(generated, encoding="utf-8")
